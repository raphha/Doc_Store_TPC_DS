...
<query8>{
  let $m := map:map()
  let $build :=
    for $sales in doc()[d_dow=1 and d_year>=1998 and d_year<=2000]/store_sales[(customer/hd_dep_count=5 or customer/hd_vehicle_count>2) and s_number_employees>=200 and s_number_employees<=295]
