...
<query8>{
  let $m := map:map()
  let $build :=
    for $sales in doc()[hd_dep_count=5 or hd_vehicle_count>2]/store_sales[date/d_dow=1 and date/d_year>=1998 and date/d_year<=2000 and s_number_employees>=200 and s_number_employees<=295]
