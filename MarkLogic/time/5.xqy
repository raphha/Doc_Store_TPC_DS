...
<query5>{
  let $m := map:map()
  let $build :=
    for $sales in doc()/store_sales[(item/i_category="Music" or item/i_category="Home" or item/i_category="Sports") and date/d_month_seq>=1200 and date/d_month_seq<=1211]
