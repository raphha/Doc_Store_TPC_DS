...
<query8>{
  let $m := map:map()
  let $build :=
    for $sales in doc()/store_sales[(customer/hd_dep_count=5 or customer/hd_vehicle_count>2) and s_number_employees>=200 and s_number_employees<=295 and date/d_dow=1 and date/d_year>=1998 and date/d_year<=2000]
