...
<query6>{
  let $m := map:map()
  let $build :=
    for $sales in doc()/store_sales[(customer/hd_dep_count=4 or customer/hd_vehicle_count=2) and s_city="Midway" and date/d_dom>=20 and date/d_dom<=21 and date/d_year>=1999 and date/d_year<=2001]
