...
<query6>{
  let $m := map:map()
  let $build :=
for $doc in cts:search(doc()/store_sales,
...
cts:path-range-query("/store_sales/s_city", "=", "Midway")
 ...
 let $c_last_name:= $doc/customer/c_last_name
    let $c_first_name:= $doc/customer/c_first_name
    let $ca_city:= $doc/customer/ca_city
    let $ss_ticket_number:= $doc/ss_ticket_number
    let $key := concat($c_last_name, ';', $c_first_name, ';', $ca_city, ';', $ss_ticket_number)
    return map:put(
      $m, $key, sum((
        map:get($m, $key),
        xs:float($doc/ss_ext_sales_price))))
